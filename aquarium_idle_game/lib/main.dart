...
class AquariumIdleGame extends StatelessWidget {
  final GoRouter _router = GoRouter(
    routes: [
      GoRoute(path: '/', builder: (context, state) => AppScaffold(child: AquariumScreen())),
      GoRoute(path: '/shop', builder: (context, state) => AppScaffold(child: ShopScreen())),
      GoRoute(path: '/upgrades', builder: (context, state) => AppScaffold(child: UpgradesScreen())),
    ],
  );
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        ...
        BlocProvider(create: (context) => ShopScreenCubit(ShopScreenState(coinCount: 0, tabBarIndex: 0))),
        BlocProvider(create: (context) => AquariumScreenCubit(AquariumScreenState())),
      ],
